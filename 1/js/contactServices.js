...
.factory('contactService', ['$http', function($http){
	return {
		contactsList: function(callback){
			$http.get('views/contacts/contacts.php?action=list').success(callback);
		},
		contactsDetails: function(id, callback){
			$http.get('views/contacts/contacts.php?action=detail&id=' + id).success(callback);
		},
		addContacts: function(readyFormData, callback){
			$http.post('views/contacts/contacts.php?action=add', readyFormData, { transformRequest: angular.identity, headers: { "Content-Type": undefined } }).success(callback);
		},
		editContact: function(id, readyFormData, callback){
			$http.post('views/contacts/contacts.php?action=edit&id=' + id, readyFormData, { transformRequest: angular.identity, headers: { "Content-Type": undefined } }).success(callback);
		},
		deleteContact: function(id, callback){
			$http.post('views/contacts/contacts.php?action=delete&id=' + id).success(callback);
		}
	}
}]);
